...
function setHistory(tab_url,max_char) {
    if(tab_url.length >= max_char) {
        historyContent += "<span class='history_date_text'>Tarih: " + getDateAndTime("DateAndTime") + "</span><br>" + tab_url.substring(0,max_char) + "...";
    } else{
        historyContent += "<span class='history_date_text'>Tarih: " + getDateAndTime("DateAndTime") + "</span><br>" + tab_url;
    }
    historyContent += "<div class='border'></div>";
    document.getElementById("history_content").innerHTML = historyContent;
    setStorage();
    loadStorage();
}